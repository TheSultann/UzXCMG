...
export const NewsFeedSection = ({
  ...
  const goPrev = () => setActiveIndex((currentIndex) => Math.max(currentIndex - 1, 0));
  const goNext = () => setActiveIndex((currentIndex) => Math.min(currentIndex + 1, maxIndex));
        ...
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex h-[38px] w-[38px] items-center justify-center border border-[#dddddd] bg-white text-[#2d2d2d] transition-all duration-300 hover:-translate-y-[1px] hover:border-[#c8c8c8] hover:bg-[#fafafa] disabled:cursor-not-allowed disabled:opacity-45 disabled:hover:translate-y-0"
            ...
            <Link
              to="/news"
              className="ml-1 inline-flex h-[38px] items-center justify-center bg-[#2d2d2d] px-[18px] text-[11px] font-semibold uppercase tracking-[0.04em] text-white transition-all duration-300 hover:-translate-y-[1px] hover:bg-black hover:shadow-[0_12px_20px_rgba(45,45,45,0.22)]"
            >
              Barcha yangiliklar
            </Link>
          </div>
              ...
              >
                ...
                >
                  <img
                    src={item.image}
                    alt={item.title}
                    className="h-[150px] w-full object-cover transition-transform duration-500 group-hover:scale-[1.05]"
                  />
                  ...
                  <div className="px-4 pb-4 pt-3">
                    ...
                    <h3 className="mb-3 text-[16px] font-medium leading-[1.45] text-[#222222]">
                      {item.title}
                    </h3>
                    <p className="text-[13px] leading-[1.55] text-[#666666]">{item.excerpt}</p>
                  </div>
                </Link>
              </div>
            ))}
